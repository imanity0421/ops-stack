...
def create_session_db(settings: "Settings") -> "BaseDb | None":
    ...
    优先级：
    1. ``AGENT_OS_SESSION_DB_URL`` 非空：按 scheme 选择 ``SqliteDb`` / ``PostgresDb`` / ``RedisDb``；
       无 ``://`` 的字符串视为**本地文件路径**（仍走 Sqlite 文件）。
    2. 否则使用 ``AGENT_OS_SESSION_DB_PATH`` 指向的 Sqlite 文件（默认 ``data/agno_session.db``）。
    """
    ...
    if "://" in lower:
        ...
        msg = f"不支持的 AGENT_OS_SESSION_DB_URL scheme: {raw!r}（支持 sqlite:、postgres(ql)://、redis://）"
