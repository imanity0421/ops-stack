...
def backup_main(argv: list[str] | None = None) -> int:
    """CLI 入口（``python -m agent_os.backup_data_core`` 或脚本包装）。"""
    import argparse

    p = argparse.ArgumentParser(description="打包 agent-os-runtime 本地 data/ 候选文件")
    ...
    p.add_argument("--name-prefix", default="agent_os_data")
    args = p.parse_args(argv)

    here = Path(__file__).resolve()
    # ``src/agent_os/backup_data_core.py`` → 仓库根为 parents[2]
