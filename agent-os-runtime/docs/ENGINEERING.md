...
| **Hindsight** | **`HindsightStore`**：`data/hindsight.jsonl`（`AGENT_OS_HISTORICAL_PATH`）；含 `feedback` 与 `lesson`；检索 `search_hindsight` / 工具 `search_past_lessons`。支持 **`supersedes_event_id`**（被取代的 `event_id` 不再召回）、**同类正文合并**与 **`weight_count`** 频次权重（可 `AGENT_OS_HISTORICAL_ENABLE_FREQ_MERGE=0` 关闭合并与加分，仍保留 supersedes 过滤）；见 [MEMORY_SYSTEM_V2.md](MEMORY_SYSTEM_V2.md) |
