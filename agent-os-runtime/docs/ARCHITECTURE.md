...
- **离线 Graphiti 入库**：独立 CLI `agent-os-runtime graphiti-ingest`（需 Neo4j + OpenAI），与运行时只读解耦。

## 质量与旁路

- **端到端规则门**：`agent-os-runtime eval <case.json>`（Golden rules，无 LLM）；库入口 `agent_os.evaluator.e2e`。
- **交付前正则抽检**：`check_delivery_text`（`AGENT_OS_GOLDEN_RULES_PATH`）。
- **无 Neo4j 的领域知识追加**：`agent-os-runtime knowledge-append-jsonl` → 与 `AGENT_OS_KNOWLEDGE_FALLBACK_PATH` 同格式。
