...
| 4 | **Resume Trace + `/context` 集成** | done-local @ 2026-04-30 (pytest + ruff passed, commit pending) | resume 装配 trace 字段、`/context` JSON 显示 `resume_diagnostics`（含 connect/fork 决策路径、`deliverable_inline_level` 命中档位、`voice_pack_skipped` 标志）、降级链 `full → digest+tail_n` 命中观测 | Battle 1 |
