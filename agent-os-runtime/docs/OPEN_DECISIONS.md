...
| 3 | **Artifact CoW v0** | done-local @ 2026-04-30 (pytest + ruff passed, commit pending) | `originating_session_id` 在 `ArtifactStore` 启用、CoW 触发规则（同 session 原地 update vs 跨 branch 强制复制）、原子事务边界（与 [ARCHITECTURE.md](ARCHITECTURE.md) §1.3 一致） | Battle 2 |
