...
本文档定义 ops-agent 在「全局层按 user 存储参考案例库」需求下的最终落地方案：**整存整取**、用于 **Dynamic Few-Shot（语感参考）**，与 Mem0/Hindsight/Graphiti 并列为第四类存储能力。

> 设计约束（来自产品/架构要求）
>
> - **Factory 不写数据库查询代码**：`agent/factory.py` 只负责创建 Agent、挂载工具与固定 instructions；所有 LanceDB 读写封装在 `ops_agent/knowledge/asset_store.py`。
...
- 新增 `ops-agent asset-ingest <input>`：
...
### 5.1 开关（建议环境变量）
- `OPS_ENABLE_ASSET_STORE`：是否启用案例库（读工具 + store 初始化）
- `OPS_ASSET_STORE_PATH`：本地 LanceDB 路径（或 URI）
- `OPS_ENABLE_HINDSIGHT`：是否启用 Hindsight（存储 + 工具）
- `OPS_ENABLE_MEM0_LEARNING`：是否允许工具写入 Mem0（关闭则不挂载 record 工具；读可保留）
...
- **近似去重**（可选 `OPS_ASSET_NEAR_DEDUP_L2_MAX`）：正文略改但 `retrieval_text` 极近时，用向量 L2 拦截重复；不设环境变量则只做强指纹去重。

## 8. 硬合规（每 skill 一份规则）

- 环境变量 `OPS_SKILL_COMPLIANCE_DIR`，文件 `<skill_id>.json`，与 `OPS_GOLDEN_RULES_PATH` 同结构（`pattern` + `message`）。
...
- `ops-agent asset-rm --case-id <id>` 删除单条。
- `ops-agent asset-rm --client-id <c> --skill <s> --all-skill` 删除该 tenant 下该 skill 的全部案例行（**危险**，用于你描述的「大量垃圾入库后一次性回退」）。
