...
需 **`OPS_ENABLE_ASSET_STORE=1`**，且正文满足入库管线最小长度（默认约 200 字，见 `asset_ingest`）。无 **`lancedb`** 时接口会 500。

调试可设 **`OPS_INGEST_ALLOW_LLM=0`** 关闭裁判/特征 LLM（仅最小可检索字段），仍走合规与去重。
