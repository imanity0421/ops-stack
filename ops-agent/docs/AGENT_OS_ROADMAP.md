...
本文档是 **ops-agent** 在「CLI + 最终 Web/API 对外、持续新增 Skill、商业/运营向交付」目标下的**定版**实施蓝图，与 [ENGINEERING.md](ENGINEERING.md) 中的运行时边界一致，**不**覆盖账号/计费/多租户控制台等完整商业化（见 ENGINEERING §1.2）。
...
|:--|:--|:--|:--|:--|
| **Sprint 1**<br>底层重构<br>*(可并行)* | **P0** | **1. Skill 扩展协议**<br>(Tool Registry) | 明确 **允许加载的 skill 根目录白名单**（如仅 `ops_agent.agent.skills.*` 或 `OPS_AGENT_SKILLS_ROOT` 下子包）。`get_agent` **仅**做「从注册表/扫描结果组装 `incremental_tools`」，**不**写入各 Skill 业务代码。动态 import 必须 **importlib + 路径校验**，禁止从任意路径加载。 | 新增一个**测试用 Skill 包**（如 `toy_skill`），**不修改 `factory.py` 内与业务相关的分支**即可注册运行（允许**集中一行**的「从 env 读 skills 包列表」等**纯框架**改动）；对非白名单路径/模块尝试加载时 **明确报错** 并拒绝执行。 |
...
| **Sprint 4**<br>长效治理 | **P3** | **7. Skill 专项评测** | 基于现有 **`ops-agent eval` / pytest** 框架，按 **skill_id** **拆分 fixture** 与门禁，**不** 引入「两套不同打分引擎」；**隔离** 修改公共层时对单 Skill 的回归。 | `pytest` 中**短视频、商业/运营**等至少两套 **独立目录或 marker**，改 A 用例不依赖 B 的数据；CI 中可**选择性**只跑某 skill 集。 |
...
| 步骤 | 方法 |
|------|------|
| 约定目录 | 每个 Skill 一个 Python 子包，入口函数固定签名，如 `register_tools(skill_id) -> list[Tool]` 或由 manifest `skill_id` 映射到 `ops_agent.data.skill_manifests` + `OPS_AGENT_MANIFEST_DIR`。 |
| 白名单 | 仅允许从 **配置枚举的包前缀** 或 **env: `OPS_SKILL_LOAD_ALLOWLIST`（模块名列表）** 加载；`importlib.import_module` 前校验。 |
| 与工厂集成 | `get_agent` 内**仅**调用 `collect_incremental_tools(eff_skill_id, settings)`，内部完成白名单 + import；**禁止**在 factory 中写 `if skill == "x": ...`。 |
| 测试 | 增加 `toy_skill` 或 `test_skill` 包与最小 manifest；单测里模拟「非法模块名」import 失败。 |
...
| 单机 | 本地 Sqlite/文件，路径由 `OPS_SESSION_DB_PATH` 等 env 管理。 |
...
| 项目 | 方法 |
|------|------|
| Per-skill eval | 目录如 `tests/e2e/short_video/`、`tests/e2e/business/`，每目录独立 `cases/*.json` + 可选 `golden_rules`；`pytest` marker `@pytest.mark.skill_short_video`。 |
...
| 周次（参考） | 动作 |
|--------------|------|
| W1–W2 | Sprint 1：Skill 白名单 + `toy_skill`；Storage 抽象 + Web 联调 F5+重启。 |
...
| 能力 | 主要落点（现状或规划） |
|------|------------------------|
| 工厂 | `ops_agent.agent.factory` |
| 会话落库 (Agno `db`) | `ops_agent.agent.session_db`（`create_session_db`）、`Settings` 中 `OPS_ENABLE_SESSION_DB` 等；Web：`GET /api/session/messages` |
| 宪法 + 结构化交付 | `ops_agent.agent.constitutional`；`manifest_output` + `planning_draft`（`structured_v1`）；验收表 `docs/examples/constitutional_test_cases.md` |
| 可观测 + 摄入网关 | `ops_agent.observability`；`ingest_gateway`；Web：`OPS_OBS`、`POST /ingest`；样例 `docs/examples/ingest_post_samples.md` |
| Per-skill eval + 备份 | `tests/skills/*` + markers；`backup_data_core` / `scripts/backup_data.py`；`docs/DATA_BACKUP.md` |
| 平台工具 + incremental | `ops_agent.agent.tools` + `ops_agent.agent.skills` |
| 记忆 | `ops_agent.memory.*`、`MemoryController` |
| 知识/案例 | `ops_agent.knowledge.*`、Asset Store 文档 [ASSET_STORE.md](ASSET_STORE.md) |
| Web 示例 | `examples/web_chat_fastapi.py` |
| 配置 | `ops_agent.config.Settings` |
...
*文档版本：定版 v1.0 | 与 ops-agent 仓库同步维护*
