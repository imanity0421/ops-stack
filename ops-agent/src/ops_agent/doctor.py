...
def run_doctor(*, strict: bool = False) -> int:
    ...
    if s.knowledge_fallback_path:
        if s.knowledge_fallback_path.is_file():
            _ok(f"OPS_KNOWLEDGE_FALLBACK_PATH 存在: {s.knowledge_fallback_path}")
        else:
            _warn(f"OPS_KNOWLEDGE_FALLBACK_PATH 指向的文件不存在: {s.knowledge_fallback_path}")

    handoff = os.getenv("OPS_HANDOFF_MANIFEST_PATH")
    if handoff:
        p = Path(handoff)
        if p.is_file():
            try:
                ...
                _ok(f"OPS_HANDOFF_MANIFEST_PATH 可读: {p} (lessons≈{n})")
    ...
    if mp:
        pp = Path(mp)
        if pp.is_file():
            _ok(f"OPS_MCP_PROBE_FIXTURE_PATH 存在: {pp}")
        else:
            _warn(f"OPS_MCP_PROBE_FIXTURE_PATH 指向的文件不存在: {pp}")

    gr = os.getenv("OPS_GOLDEN_RULES_PATH")
    if gr:
        gp = Path(gr)
        if gp.is_file():
            _ok(f"OPS_GOLDEN_RULES_PATH 存在: {gp}")
        else:
            _warn(f"OPS_GOLDEN_RULES_PATH 指向的文件不存在: {gp}")
