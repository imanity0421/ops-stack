"""
Skill 包动态加载：仅允许自 ``ops_agent.agent.skills.<name>`` 子包且 name 在环境白名单内。
...
_SKILLS_PREFIX = "ops_agent.agent.skills"
...
def get_incremental_tools_for_skill(
    ...
    *,
    ...
    - ``skill_id`` 在白名单：``importlib.import_module("ops_agent.agent.skills.<skill_id>")``，调用 ``get_tools()``。
    """
    from ops_agent.config import Settings
