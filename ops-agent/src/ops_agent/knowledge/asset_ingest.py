...
from ops_agent.knowledge.asset_store import (
    AssetCase,
    AssetStatus,
    AssetStore,
...
class IngestOptions:
    ...
    allow_llm: bool = True
    #: 每 skill 合规规则目录（``OPS_SKILL_COMPLIANCE_DIR``）；未设置则不做硬合规
...
def _llm_gatekeeper(raw: str, *, opt: IngestOptions) -> tuple[AssetStatus, str | None]:
    ...
    prompt = (
        "你是参考案例库的入库裁判。判断下面文本是否是“可复用的整案案例”（例如短视频脚本、完整文案、完整策划方案），"
...
def ingest_text(raw: str, *, store: AssetStore, opt: IngestOptions) -> dict[str, Any]:
    ...
    near_max = (os.getenv("OPS_ASSET_NEAR_DEDUP_L2_MAX") or "").strip()
