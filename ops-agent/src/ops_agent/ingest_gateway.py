...
from ops_agent.knowledge.asset_ingest import IngestOptions, ingest_text
...
def _ingest_allow_llm() -> bool:
    return os.getenv("OPS_INGEST_ALLOW_LLM", "1").lower() not in ("0", "false", "no")
    ...
    *,
    ...
    if t == "hindsight":
        if controller.hindsight_store is None:
            raise ValueError("Hindsight 未启用（OPS_ENABLE_HINDSIGHT=0 或存储未初始化）")
        fact = UserFact(
    ...
    if not settings.enable_asset_store:
        raise ValueError("未启用 Asset Store（OPS_ENABLE_ASSET_STORE=0）")
